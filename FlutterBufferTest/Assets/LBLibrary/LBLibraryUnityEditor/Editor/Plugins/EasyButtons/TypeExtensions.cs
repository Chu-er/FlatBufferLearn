namespace LIBII.EasyButtons.Editor.Utils
{
    ...
    public static class TypeExtensions
    {
        ...
        public static bool IsUnitySerializable(this Type type)
        {
            ...
            if (type.IsArray && type.GetElementType().IsUnitySerializable())
                return true;

            if (type.IsGenericType)
            {
                if (type.FullName.Contains("System.Collections.Generic.List") && type.GetGenericArguments().Length == 1 &&
                    type.GetGenericArguments()[0].IsUnitySerializable())
                    return true;
            }
